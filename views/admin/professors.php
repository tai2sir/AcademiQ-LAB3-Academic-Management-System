...
<div class="row g-4">
    <div class="col-lg-5">
        <div class="card border-0 shadow-sm h-100">
            <div class="card-header bg-white fw-semibold"><?= $edit ? 'Edit professor' : 'New professor' ?></div>
            <div class="card-body">
                <?php if ($edit): ?>
                    <form method="post" class="vstack gap-3">
                        ...
                        <div class="d-flex gap-2">
                            ...
                            <a class="btn btn-outline-secondary" href="<?= e(url('admin_professors')) ?>">Cancel</a>
                        </div>
                    </form>
                <?php else: ?>
                    <form method="post" class="vstack gap-3">
    ...
    <div class="col-lg-7">
        <div class="card border-0 shadow-sm">
            <div class="card-header bg-white">
                <form class="row g-2 align-items-center" method="get" action="<?= e(url('admin_professors')) ?>">
                    <input type="hidden" name="page" value="admin_professors">
                    <div class="col">
                    ...
                    <?php foreach ($professors as $p): ?>
                        <tr>
                            ...
                            <td class="text-end text-nowrap">
                                <a class="btn btn-sm btn-outline-primary" href="<?= e(url('admin_professors', ['edit' => (string) $p['id'], 'q' => $search, 'p' => (string) $pagination['page']])) ?>">Edit</a>
                                <form method="post" class="d-inline" onsubmit="return confirm('Delete professor?');">
                ...
                <div class="card-footer bg-white d-flex justify-content-between align-items-center">
                    ...
                    <div class="btn-group">
                        ...
                        <a class="btn btn-sm btn-outline-secondary <?= $pagination['page'] <= 1 ? 'disabled' : '' ?>"
                           href="<?= e(url('admin_professors', ['p' => (string) $prev, 'q' => $search])) ?>">Prev</a>
                        <a class="btn btn-sm btn-outline-secondary <?= $pagination['page'] >= $pagination['total_pages'] ? 'disabled' : '' ?>"
                           href="<?= e(url('admin_professors', ['p' => (string) $next, 'q' => $search])) ?>">Next</a>
                    </div>
                </div>
            <?php endif; ?>
        </div>
